...
ElectricPowerProvider.destroy_all

count = 0
CSV.foreach('db/csv/test/electric_power_provider.csv', headers: true) do |row|
  ElectricPowerProvider.create!(id: count += 1,
                                name: row['name'])
end

count = 0
CSV.foreach('db/csv/test/electricity_rate_plan.csv', headers: true) do |row|
  ElectricityRatePlan.create!(id: count += 1,
                              name: row['name'],
                              electric_power_provider_id: row['electric_power_provider_id'])
end

CSV.foreach('db/csv/test/basic_charge.csv', headers: true) do |row|
...
CSV.foreach('db/csv/test/usage_charge.csv', headers: true) do |row|
