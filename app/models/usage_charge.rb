...
class UsageCharge < ApplicationRecord
  ...
  validates :minimum_usage,
            ...
            uniqueness: { scope: %i[electricity_rate_plan_id max_usage],
  ...
  def cannot_minimum_less_than_maximum
    ...
    errors.add(:minimum_usage, '最大値未満の数値を入力してください。') if minimum_usage >= max_usage
  end

  def cannot_maximum_greater_than_minimum
    return if minimum_usage.blank? || max_usage.blank?

    errors.add(:max_usage, '最小値より大きい数値を入力してください。') if max_usage <= minimum_usage
  end
end
