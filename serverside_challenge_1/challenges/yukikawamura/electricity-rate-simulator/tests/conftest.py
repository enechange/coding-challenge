...
from electricity_rate_simulator.model import ProFile, PlanContract, PlanUsage
...
def _test_provider_file(profile: Path):
    with open(profile, "r", encoding="utf-8") as f:
        ...
        contracts = [PlanContract(contract=contracts.get("contract"), price=contracts.get("price")) for contracts in profile_data.get("contracts")]
        usages = [PlanUsage(over=usage.get("over"), until=usage.get("until"), price=usage.get("price")) for usage in profile_data.get("usage")]
        return ProFile(provider=profile_data.get("provider"), plan=profile_data.get("name"), contracts=contracts, usage=usages)
