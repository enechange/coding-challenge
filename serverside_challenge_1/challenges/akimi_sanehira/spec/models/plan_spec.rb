...
RSpec.describe Plan, type: :model do
  ...
  it "有効なデータで登録できる" do
    expect(create(:plan, provider: provider)).to be_valid
  end
  ...
  it "名前がなければ登録できない" do
    not_valid_plan_1 = build(:plan, provider: provider, name: "")
    expect(not_valid_plan_1).to_not be_valid

    not_valid_plan_2 = build(:plan, provider: provider, name: "     ")
    expect(not_valid_plan_2).to_not be_valid
  end

  it "複数の基本料金設定を登録することができる" do
    plan_ex = create(:plan, provider: provider)
    3.times do
