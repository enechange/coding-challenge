...
class ElectricityPlan
  attr_reader :provider, :name, :basic_charge_table, :energy_use_charge_table

  def initialize(provider, name, basic_charge_table, energy_use_charge_table)
    @provider = provider
    @name = name
    @basic_charge_table = basic_charge_table
    @energy_use_charge_table = energy_use_charge_table
  end

  def calculate_price(contract_amperage, consumption)
    basic_charge = basic_charge_table.calculate_charge(contract_amperage)
    energy_use_charge = energy_use_charge_table.calculate_charge(consumption)
